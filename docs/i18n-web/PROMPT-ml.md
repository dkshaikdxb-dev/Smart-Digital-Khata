# Malayalam (`ml`) — web console translation

232 strings, split into 2 files under `web-ml-parts/`.
...
Translate the attached CSV into **Malayalam**, written in the **Malayalam script**.
...
2. **One script only.** Every character must be Malayalam, except placeholders,
...
| paid | അടച്ചു |
| on khata | ഖാതയിൽ |
| pay online | ഓൺലൈൻ അടവ് |
| you owe | നിങ്ങൾ കൊടുക്കാനുള്ളത് |
| staff | ജീവനക്കാർ |
| free | സൗജന്യം |
...
5. **Plain spoken register.** Where the natural spoken word is an English
   ...
   mobile — use it written in Malayalam, rather than forcing a pure word nobody
   uses in a shop.
...
Save the reply as `web-ml-NN.csv` next to the part it answers and tell me.
...
placeholders survive exactly, that the text is in Malayalam and carries no
other Indic script, that no row came back in English, and that nothing contains
a replacement character.

## Parts

- `web-ml-parts/web-ml-01.csv` — 124 strings (acc.language … oedit.wasNow)
- `web-ml-parts/web-ml-02.csv` — 108 strings (open.addClosure … voice.err.network)
