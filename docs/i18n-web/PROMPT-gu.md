...
Translate the attached CSV into **Gujarati**, written in the **Gujarati script**.
...
2. **One script only.** Every character must be Gujarati, except placeholders,
...
| paid | ચૂકવાયું |
| on khata | ખાતામાં |
| pay online | ઓનલાઇન ચૂકવણી |
| you owe | તમારે આપવાના |
| staff | સ્ટાફ |
| free | મફત |
| open | ખુલ્લી |
| closed | બંધ |
...
5. **Plain spoken register.** Where the natural spoken word is an English
   ...
   mobile — use it written in Gujarati, rather than forcing a pure word nobody
   uses in a shop.
...
Save the reply as `web-gu-NN.csv` next to the part it answers and tell me.
...
placeholders survive exactly, that the text is in Gujarati and carries no
