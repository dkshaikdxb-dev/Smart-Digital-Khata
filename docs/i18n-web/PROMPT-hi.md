# Hindi (`hi`) — web console translation

21 strings, split into 1 files under `web-hi-parts/`.
**Upload ONE part per conversation turn and paste the prompt below each time.**
Asking for all 21 in one reply is where a model starts dropping rows near
...
Translate the attached CSV into **Hindi**, written in the **Devanagari script**.
...
Save the reply as `web-hi-NN.csv` next to the part it answers and tell me.
...
- `web-hi-parts/web-hi-01.csv` — 21 strings (c.buyItAgain … stale.title)
