# Kannada (`kn`) — web console translation

232 strings, split into 2 files under `web-kn-parts/`.
...
Translate the attached CSV into **Kannada**, written in the **Kannada script**.
...
2. **One script only.** Every character must be Kannada, except placeholders,
...
| paid | ಪಾವತಿಸಲಾಗಿದೆ |
| on khata | ಖಾತೆಯಲ್ಲಿ |
| pay online | ಆನ್‌ಲೈನ್ ಪಾವತಿ |
| you owe | ನೀವು ಕೊಡಬೇಕಾದದ್ದು |
| staff | ಸಿಬ್ಬಂದಿ |
| free | ಉಚಿತ |
| open | ತೆರೆದಿದೆ |
| closed | ಮುಚ್ಚಿದೆ |
...
5. **Plain spoken register.** Where the natural spoken word is an English
   ...
   mobile — use it written in Kannada, rather than forcing a pure word nobody
   uses in a shop.
...
Save the reply as `web-kn-NN.csv` next to the part it answers and tell me.
...
placeholders survive exactly, that the text is in Kannada and carries no
other Indic script, that no row came back in English, and that nothing contains
a replacement character.

## Parts

- `web-kn-parts/web-kn-01.csv` — 124 strings (acc.language … oedit.wasNow)
- `web-kn-parts/web-kn-02.csv` — 108 strings (open.addClosure … voice.err.network)
