# Tamil (`ta`) — web console translation

232 strings, split into 2 files under `web-ta-parts/`.
...
Translate the attached CSV into **Tamil**, written in the **Tamil script**.
...
2. **One script only.** Every character must be Tamil, except placeholders,
...
5. **Plain spoken register.** Where the natural spoken word is an English
   ...
   mobile — use it written in Tamil, rather than forcing a pure word nobody
   uses in a shop.
...
Save the reply as `web-ta-NN.csv` next to the part it answers and tell me.
...
placeholders survive exactly, that the text is in Tamil and carries no
other Indic script, that no row came back in English, and that nothing contains
a replacement character.

## Parts

- `web-ta-parts/web-ta-01.csv` — 124 strings (acc.language … oedit.wasNow)
- `web-ta-parts/web-ta-02.csv` — 108 strings (open.addClosure … voice.err.network)
