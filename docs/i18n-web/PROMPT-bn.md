# Bengali (`bn`) — web console translation

566 strings, split into 5 files under `web-bn-parts/`.
...
Translate the attached CSV into **Bengali**, written in the **Bengali script**.
...
2. **One script only.** Every character must be Bengali, except placeholders,
...
5. **Plain spoken register.** Where the natural spoken word is an English
   ...
   mobile — use it written in Bengali, rather than forcing a pure word nobody
   uses in a shop.
...
Save the reply as `web-bn-NN.csv` next to the part it answers and tell me.
...
placeholders survive exactly, that the text is in Bengali and carries no
other Indic script, that no row came back in English, and that nothing contains
a replacement character.

## Parts

- `web-bn-parts/web-bn-01.csv` — 128 strings (acc.language … credits.title)
- `web-bn-parts/web-bn-02.csv` — 126 strings (csfaq.subtitle … dlv.unassigned)
- `web-bn-parts/web-bn-03.csv` — 121 strings (err.accountStopped … open.tomorrowAt)
- `web-bn-parts/web-bn-04.csv` — 120 strings (ord.advanceCol … share.title)
- `web-bn-parts/web-bn-05.csv` — 71 strings (snl.auto … voice.err.network)
