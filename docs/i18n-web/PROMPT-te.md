# Telugu (`te`) — web console translation

232 strings, split into 2 files under `web-te-parts/`.
...
Translate the attached CSV into **Telugu**, written in the **Telugu script**.
...
2. **One script only.** Every character must be Telugu, except placeholders,
...
| paid | చెల్లించబడింది |
| on khata | ఖాతాలో |
| pay online | ఆన్‌లైన్ చెల్లింపు |
| you owe | మీరు ఇవ్వాల్సింది |
| staff | సిబ్బంది |
| free | ఉచితం |
| open | తెరిచి ఉంది |
| closed | మూసి ఉంది |
...
5. **Plain spoken register.** Where the natural spoken word is an English
   ...
   mobile — use it written in Telugu, rather than forcing a pure word nobody
   uses in a shop.
...
Save the reply as `web-te-NN.csv` next to the part it answers and tell me.
...
placeholders survive exactly, that the text is in Telugu and carries no
other Indic script, that no row came back in English, and that nothing contains
a replacement character.

## Parts

- `web-te-parts/web-te-01.csv` — 124 strings (acc.language … oedit.wasNow)
- `web-te-parts/web-te-02.csv` — 108 strings (open.addClosure … voice.err.network)
