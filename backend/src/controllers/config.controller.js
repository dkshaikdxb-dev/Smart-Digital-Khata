...
// The owner "Share your shop" poster toggle (batch SOCIAL1), read LIVE from
// platform_settings the same way as the voice flag. DEFAULT ON: only OFF when
// stored explicitly as 'false'; a missing row or ANY error → true, so the free,
// on-device (Canvas + qrcode, no Meta API, no credentials) share poster never
// disappears because of a transient DB hiccup. Migration 0058 seeds 'true'.
// NOTE: meta_autopost_enabled (the PARKED, licensed Meta Graph auto-post +
// catalogue sync path) is deliberately NOT exposed here — it is a flag-off seam,
// not a live feature, so the public config never advertises it.
async function getSocialShareEnabled() {
  try {
    const r = await query(
      `SELECT value FROM platform_settings WHERE key = 'social_share_enabled'`
    );
    const v = r.rows[0] && r.rows[0].value;
    return v !== 'false'; // unset / anything-but-'false' → enabled
  } catch (_e) {
