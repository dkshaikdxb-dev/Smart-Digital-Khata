// The shopkeeper app's screen tree.
//
// Moved OUT of App.js verbatim, and that move is the whole point: App.js now
// require()s this module only AFTER the platform accent has been written into
// src/theme.js, because every StyleSheet.create below and in the screens it
// imports is evaluated at import time and reads `colors.accent` as a property
// at that moment. A static `import` at the top of App.js would be hoisted and
// run before the boot gate, and the app would paint the shipped green.
//
...
import { auth, getToken, getRole, setUnauthorizedHandler } from './services/api';
...
// The owner flavor's LanguageProvider lives HERE rather than in App.js, so the
// boot gate imports nothing from this tree — not the screens, not the provider,
// not the dictionary. Everything below this file's first line runs after the
// accent is in place. The consumer flavor keeps its own separate provider
// inside ConsumerApp; the two are untouched by each other.
