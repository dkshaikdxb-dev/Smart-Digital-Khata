// Ready-time promise (batch B) — the RENDERING side of the one truth, shared by
// the owner web console and the consumer PWA (both served out of this app).
//
...
// The native mirror of this file is mobile-app/src/lib/orderEta.js, with the
// same key names, so the four surfaces never word the same fact differently.
//
// Deliberately NOT server-side: "late" is a comparison against the reader's own
// clock. Computing it in the API would need a column kept fresh by a job, and an
// offline client would still disagree with it.
...
const PENDING_PROMISE = new Set(['pending', 'accepted', 'preparing']);
...
export function etaState(order, now) {
  ...
  if (!PENDING_PROMISE.has(order.status)) return 'settled';
  const t = now ? new Date(now).getTime() : Date.now();
  return at.getTime() >= t ? 'promised' : 'late';
}

/** A locale tag for Intl from the app's 2-letter language. India-first. */
